...
\item  \emph{O Homem-Aranha está tentando entender o plano maligno do Dr. Octopus.} Se o Dr. Octopus conseguir obter urânio, ele chantageará a cidade. Estou certo disso, porque se o Dr. Octopus conseguir obter urânio, ele poderá fazer uma bomba suja e, se ele puder fazer uma bomba suja, chantageará a cidade.
...
A expressão ``as maçãs são vermelhas ou as jabuticabas são pretas'' é uma sentença em português e a expressão ``$(M \eor J)$'' é uma sentença da LVF.
...
(Nós as chamaremos de sentenças \emph{atômicas}.)
...
a regra para geração de novos elementos e o fechamento são dados pelas seguintes cláusulas:
	\begin{ebullet}
		\item Se $x$ is meu ancestral, então os pais de $x$ são meus ancestrais.
...
A conjunção `\eand', por sua vez, é o primeiro conectivo que ocorre à direita do ponto em que a contagem está em 1.
...
A ideia é apenas facilitar a leitura.
...
Ela não é uma expressão específica da LVF, como `$A$', mas uma variável da metalinguagem (metavariável) que se refere genericamente a qualquer expressão da LVF, tal como $$A,\ \ B,\ \  (A \eif (Q \eand R)),\ \  \enot\enot(B_3 \eif Z),\ \ldots$$
