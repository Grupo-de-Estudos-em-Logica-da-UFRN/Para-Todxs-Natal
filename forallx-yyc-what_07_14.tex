...
É esta ideia de garantia infalível, de justificativa confiável, que está por trás da definição de argumento váido.
...
\end{earg}
Repare que em nenhum destes dois argumentos a conclusão é verdadeira.\footnote{
...
Este é também o motivo pelo qual a  conclusão é falsa.
O último a deixar a mansão não ``foi o motorista'', como afirma a conclusão, mas o jardineiro.

Chamamos uma situação como esta, em que as premissas de um argumento são todas verdadeiras, mas a conclusão é falsa de  \define{counterexample} do argumento.
...
Tinhamos ali o seguinte argumento:
 ...
 \begin{earg}
 	...
 	\item[\therefore] Foi o motorista.
 \end{earg}
E propuzemos a seguinte situação, que mostramos ser um contraexemplo para este argumento.
...
Com estes dois elementos, a situação configura-se em um cenário hipotético que tem informação suficiente para decidir a verdade ou falsidade das três sentenças do argumento.
...
\begin{description}
	\item[situação 3:] Jordão é destro e nasceu em Natal há 21 anos.
\end{description} 
Repare que a estória contada nessa situação é minimalista.
Muito pouco é dito. Apenas que Jordão é natalense, destro e tem 21 anos.
Ou seja, está explícito na situação 3 que a segunda premissa é verdadeira e a conclusão é falsa.
...
No mundo real há natalenses canhotos.
...
O resultado aqui é que na situação 3 não temos as duas premissas verdadeiras e a conclusão falsa. Ou seja, assim como a 2, a situação 3 também não é um contraexemplo ao nosso argumento.

Mesmo que você seja a pessoa mais imaginativa do mundo e passe a vida tentando, sem trapacear\footnote{
	Trapacear aqui seria modificar o significado das palavras lógicas `todos' ou `é' (mais adiante no livro, explicaremos o que são `palavras lógicas'), ou então estipular que as outras palavras, como `natalense' ou `canhoto' ou mesmo `Jordão' significam coisas diferentes em sentenças diferentes.
...
Na situação 2, por exemplo, um meteorito caiu em Natal e como consequência inexplicável deste fenômeno todas as pessoas nascidas na cidade desde então são canhotas.
...
Então, se aceitarmos a suposição de que as situações devem respeitar as leis da física, não conseguiremos produzir nenhum contraexemplo a este argumento, que será, por isso, considerado válido.
...
Já se permitimos situações em que ser uma oftalmologista significa algo diferente de ser uma médica de olhos, então é claro que a premissa deste argumento pode ser verdadeira e a conclsão falsa.
...
Aqui também a lógica tomará a posição mais liberal e, de modo geral, admitirá situações que violam as relações conceituais e o significado das palavras.
Então, de um ponto de vista lógico, o argumento acima sobre Jussara é inválido, porque são admissíveis situações em que os conceitos de ser oftalmologista e ser médica de olhos não são idênticos, o que possibilita a existência de contraexemplos ao argumento.
...
Estes dois exemplos de validades alternativas, ambas diferentes da validade lógica mais tradicional que estudaremos neste livro, ilustram algo muito importante:
...
\end{earg}
\end{description}
Esse argumento é conceitualmente válido. Nenhuma situação que respeite o fato de que `oftalmologista' e `médica de olhos' têm o mesmo significado será contraexemplo dele.
Mas se desrespeitarmos a equivalência entre esses dois conceitos, podemos propor, por exemplo, a seguinte situação:
`oftalmologista' significa `samambaia' e `Jussara' é o nome que dei para a samambaia da minha varanda.
Nesta situação, as duas premissas do argumento 1 são verdadeiras, mas a conclusão é falsa, porque a samambaia de minha varanda certamente não é uma médica de olhos.
...
\end{earg}
\end{description}
Esse argumento não tem contraexemplo mesmo quando permitimos mudar os significados das palavras que ocorrem nele.
...
\end{earg}
\end{description}
Aqui, `$A$', `$X$' e `$Y$' funcionam como espaços reservados para expressões apropriadas que, quando substituem `$A$', `$X$' e `$Y$', transformam este padrão em um argumento constituído por sentenças em português.
Por exemplo,
\begin{description}
\item[Argumento 3]
\begin{earg}
...
Note que este argumento também válido.
Os argumentos 2, 3 e todos os outros cuja forma lógica tem o padrão 1 são válidos.
...
Porém, mesmo que um argumento seja conceitualmente válido, se ele não tem uma fórma lógica válida, é possível encontrar outros argumentos com a mesma forma lógica,  mas que não são conceitualmente válidos. Vejamos um exemplo.
Se no argumento 1 substituirmos `Jussara' por `$A$',  `oftalmologista' por `$X$', `dentista' por `$Y$' e  `médica de olhos' por `$Z$', obtemos o seguinte padrão:
...
\end{earg}
\end{description}
Se agora tomamos este padrão e substituímos `$A$' por `Edna', `$X$' por `matemática', `$Y$' por `bióloga' e `$Z$' por `trapezista', obtemos o seguinte argumento:
\begin{description}
\item[Argumento 4]
\begin{earg}
	\item[]
...
A estratégia da lógica consistirá, então, em buscar a validade formal através de métodos capazes de reconhecer quais são as formas lógicas válidas que vão garantir a validade de todos os argumentos que as compartilham.
...
Por isso, apesar de ter uma conclusão falsa, este argumento é válido.
...
A situação que realmente ocorre, o mundo real, não tem nenhum papel especial na determinação de se um argumento é válido ou não.\footnote{
	Bem, o único caso em que a situação real parece interferir na avaliação lógica de um argumento é quando as premissas são de fato verdadeiras e a conclusão de fato é falsa.
...
A moral da história aqui é que se você quiser usar um argumento para comprovar que uma sentença é verdadeira, você tem que construir um argumento válido e com premissas verdadeiras no qual esta sentença seja a conclusão.
Um argumento com estas caracterísitcas, válido e com todas as suas premissas verdadeiras, é chamado de argumento \definepl{soundness}.
...
\factoidbox{
	\begin{description}
		\item[Usar um argumento -- {\small (comprova a verdade)}] Se você quer usar um argumento para \textit{comprovar a verdade} de uma afirmação, você precisa que esta afirmação seja a conclusão de um argumento \textit{correto}. Ou seja, você tem \textbf{duas tarefas}:
		\begin{enumerate}
			\item  você \textit{tem que} mostrar que o argumento é válido, \textbf{e}
			\item você \textit{tem que} mostrar que todas as premissas são verdadeiras.
		\end{enumerate}
		\item[Refutar um argumento -- {\small (rejeita a justificativa)}] Se você quer refutar um argumento, ou seja, mostrar que ele não é uma boa justificativa para a verdade da conclusão, você tem \textbf{duas opções}:
		\begin{enumerate}
		...
		\item[Falácia da falácia --] Cuidado!
			...
			Para comprovar que a conclusão é falsa, é preciso propor um novo argumento que seja correto (válido e com premissas verddeiras) e cuja conclusão seja a negação da conclusão do argumento refutado.\footnote{
				O erro de achar que a refutação de um argumento prova que sua conclusão é falsa é conhecido como \textit{falácia da falácia}: é uma falácia achar que o que uma pessoa diz é falso apenas porque a pessoa não tem uma boa justificativa para a sua afirmação.}
		\end{description}}
...
Apesar de bastante razoável, este argumento é inválido.
...
Antes de finalizar o capítulo, aqui vão dois alertas vocabulares:
\begin{enumerate}
	\item Você pode dizer que a conclusão \textit{se segue} das premissas, ou que é uma \textit{consequência} das premissas. Mas não diga que as premissas  \textit{inferem} a conclusão. Inferência é um raciocínio.
	É algo que nós fazemos. Então quando a conclusão se segue das premissas, \textit{nós} podemos inferir a conclusão das premissas.
...
Antes de nos dedicarmos a elas, no entanto, vale a pena mais um esclarecimento vocabular.

Apresentamos, no capítulo anterior, três noções diferentes de validade. A validade nomológica, a validade conceitual e a validade formal. *****

AQUI 



Conforme o subtítulo indica, este é um livro de lógica formal, então, na maioria dos contextos em que usamos a palavra validade, estamos nos referindo à validade formal

Quando lidamos com argumentos informais,

Já dissemos que o método formal da lógica consistirá em encontrar formas lógicas válidas.
Dado um argumento a avaliação lógica será obtida

 com as quais podemos chamar de formalmente válidos todos os argumentos que as compartilham encontrar formas lógicas válidas para aponatQuando trabalhamos com 
