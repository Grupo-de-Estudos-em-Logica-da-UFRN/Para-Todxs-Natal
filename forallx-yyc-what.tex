...
Um argumento que possui esta propriedade é chamado de \define{valid}.
...
Chamamos uma situação como esta, que prova a invalidade de um argumento, de \define{counterexample} ao argumento.
...
Isso significa, em primeiro lugar, que não será aceito como uma situação admissível para um possível contraexemplo, qualquer cenário imaginário no qual a verdade ou não de alguma sentença do argumento considerado não seja determinada.
...
Se uma situação não determina que uma sentença é verdadeira, diremos que ela determina que a sentença é \xdefine{falsa}.
...
Isso motiva a seguinte definição:
	...
	\factoidbox{
		As sentenças de um grupo são \xdefine{conjuntamente possiveis} se e somente se houver uma situação em que todas elas sejam verdadeiras.
	}
...
Uma sentença como \ref{Acontingent} que é capaz de ser verdadeira e capaz de ser falsa (em diferentes circunstâncias, é claro) é chamada \xdefine{contingente}.
...
\begin{itemize}
\item Um argumento é \define{valid} se não houver nenhuma situação em que as premissas sejam todas verdadeiras e a conclusão não seja; é \define{invalid} caso contrário.

\item Uma \define{necessary truth} é uma sentença verdadeira em todas as situações.

\item Uma \define{necessary falsehood} é uma sentença que é falsa em todas as situações.

\item Uma \xdefine{sentenca contingente} não é nem uma verdade necessária nem uma falsidade necessária; é verdadeira em algumas situações e falsa em outras.

\item Duas sentenças são \definepl{necessary equivalence} se, em qualquer situação, têm o mesmo valor de verdade; ou são ambas verdadeiras ou ambas falsas.

\item Uma coleção de sentenças é \xdefine{conjuntamente possivel} se houver uma situação em que todas sejam verdadeiras; e é \xdefine{conjuntamente impossivel} caso contrário.
\end{itemize}
