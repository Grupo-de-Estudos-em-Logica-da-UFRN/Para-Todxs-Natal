...
\part{Introdução Informal à Lógica}
\label{ch.intro}
...
\begin{description}
	...
	\item[\namedlabel{premissafalsa}{Quando as premissas \textit{não são} todas verdadeiras:}] a conclusão pode ser verdadeira ou falsa.
...
\begin{earg}
    ...
    \phantomsection %corrige link errado apesar de número de página correto
    \label{argMaidDriver}
\end{earg}
...
Em linhas bem gerais uma \define{case} é um estado de coisas possível, ou seja, é um cenário hipotético, uma estória imaginada que descreve um modo como o mundo poderia ser e que tem informação suficiente para decidir se as sentenças de um argumento são verdadeiras ou falsas nessa estória.
...
Já vimos, na Página \pageref{argMaidDriver}, um exemplo de situação. % o número da página aparece certo, mas o link leva para a primeira página da seção onde a referência ocorre argMaidDriver contraexemplo
...
Quando duas sentenças têm sempre o mesmo \define{truth value} em todas as situações, dizemos que elas são \definepl{necessary equivalence}.
